...
function showProc(c, a) {
	...
	if (a) {
		...
		$("<img src='/Template/default/Public/images/processing.gif' id='imgProc' alt='正在处理' />").insertAfter(c)
	} else {
		$(c).show();
		b.remove()
	}
...
function $a(c, p, g, l, o, a) {
	...
	j.html(o);
	k.html(c);
	var i = "/Template/default/Public/images/";
	switch (p) {
	case 1:
		i += "ico_ok.gif";
		break;
...
function IndexSendLeaveword(src) {
	var textTel = $v("txt_tel");
	var textName = $v("txt_name");
	var stxtemail = $v("txt_email");
	var textMessage = $v("txt_message");
	var _verCode = $v("txt_code");
	var err = "";
	...
	if (reg.test(textName)) {
		err += "<li style=\"height:18px\">姓名不可为空</li>";
	}
	if (reg.test(textTel)) {
		err += "<li style=\"height:18px\">电话不可为空</li>";
	} else if (!PARTTEN.test(textTel)) {
		err += "<li style=\"height:18px\">电话格式错误</li>";
	}
	if (reg.test(stxtemail)) {
		err += "<li style=\"height:18px\">邮箱不可为空</li>";
	} else if (!PTN_EMAIL.test(stxtemail)) {
		err += "<li style=\"height:18px\">邮箱格式错误</li>";
	}

	if (_verCode == null || _verCode == "") {
		err += "<li style=\"height:18px\">验证码不可为空</li>";
	}
	//alert(123);
	if (err.length > 0) {
		$a(err);
	...
	$.post("/Inquiry/submit.html", {
		...
		var sta = msg.state;//gav(msg, "state");
		var sMsg = msg.msg;//gav(msg, "msg");
		if (sta == "1") {
			emptyText("vvvvv");
			$a(sMsg, 1);
		} else {
